...
Trying a good wine is an act that as a civilization we have experienced over and over again for thousands of years. 
Many things have changed over time, more so as we started applying science and technology to winemaking. One has remained: The final word on wine quality relies on the human taste.
If making a simple correlation between that 'subjective' index (quality) and other 'objective' variables (pH, acidity, density, etc.) could shed some light on that complicated process, it definitive worth a try. 
In this project, we used the data for red wines from P.Cortez et. al [1] to correlate wine quality with some process variables in an attempt to find conditions for improving wine quality. 
